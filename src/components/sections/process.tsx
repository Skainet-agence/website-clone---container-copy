...
const ProcessCard = ({ number, title, description, features, index, isVisible }: ProcessCardProps) => (
  <div 
    className={`relative p-6 md:p-10 rounded-2xl bg-[#2a2f5a] border border-white/10 overflow-hidden group transition-all duration-700 hover:-translate-y-2 hover:shadow-[0_16px_32px_rgba(0,0,0,0.5)] hover:border-white/20 ${isVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-12 scale-95'}`}
      ...
      <div className="container mx-auto px-4 max-w-7xl">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          {processSteps.map((step, index) => (
            <ProcessCard key={index} {...step} index={index} isVisible={isVisible} />
          ))}
        </div>
      </div>
    </section>
